...
LEFT JOIN {{ref('scd2_djangomart_products')}} pr 
    ...
    AND pi.CREATED_DATE >= pr.DWH_EFFECTIVE_FROM 
    AND pi.CREATED_DATE < pr.DWH_EFFECTIVE_TO 
LEFT JOIN {{ref('scd2_djangomart_categories')}} c 
    ON pr.CATEGORY_ID = c.ID 
    AND pi.CREATED_DATE >= c.DWH_EFFECTIVE_FROM 
    AND pi.CREATED_DATE < c.DWH_EFFECTIVE_TO
